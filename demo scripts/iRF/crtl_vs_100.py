...
# load data
with open('../../data/sample_sheet.csv', 'r') as f:
    idx = np.array([r for r in csv.reader(f, delimiter = ',')])

with open('../../data/rna_norm_counts.csv', 'r') as f:
...
irfres = irf_utils.run_iRF(
    ...
    B = 50, # The number of bootstrap samples. Play around with this, see what changes
    M = 123, # number of trees (RIT) to build. Look into the effects of this parameter
    max_depth = 5,
)
